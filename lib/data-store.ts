...
export async function fetchProductsData(): Promise<ProductsData> {
  if (isFirebaseConfigured() && db) {
    try {
      ...
      // If categories are empty in Firestore, seed default categories
...
export async function syncQuoteToAdmin(entry: ContactEntry): Promise<boolean> {
  if (isFirebaseConfigured() && db) {
    try {
      await setDoc(doc(db, "contacts", entry.id), entry);
      return true;
    } catch (err) {
      console.error("Error syncing quote to Firebase Firestore:", err);
    }
  }
  return false;
}
